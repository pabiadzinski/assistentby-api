...
func main() {
	...
	api := a.ApiAssistent{
		C: a.NewClient(teamId, token, "http://app.assisntent.by/api/v1"),
	}
	...
	responseBankAccounts, _ := api.StoreBankAccount(formBankAccount)
	fmt.Printf("%+v\n", responseBankAccounts)

	respOperations, _ := api.StoreOperation(formOperation)
	fmt.Printf("%+v\n", respOperations)

	bankParams := "code=AKBBBY21802"
	responseBanks, _ := api.GetBanks(bankParams)
	fmt.Printf("%+v\n", responseBanks)
}
